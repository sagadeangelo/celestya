...
for i in range(1, N + 1):
    ...
    age_bucket = "B_26_45"  # force for now
    city = BASE_CITY
    ...
    if exists:
        ...
        UPDATE users SET
          ...
          profile_photo_key=?
        """
        params = [name, birth, age_bucket, city, lat, lon, photo_key]
        if has_updated:
            q += ", updated_at=?"
            params.append(now)
        q += " WHERE email=?"
        ...
        insert_cols = ["email", "name", "gender", "show_me", "birthdate", "age_bucket", "city", "lat", "lon", "email_verified", "profile_photo_key", "password_hash"]
        insert_vals = [email, name, "female", "male", birth, age_bucket, city, lat, lon, 1, photo_key, "dummy_hash_for_seeding"]
        if has_created:
        ...
        cur.execute(
            f"INSERT INTO users ({','.join(insert_cols)}) VALUES ({','.join(['?'] * len(insert_cols))})",
            insert_vals
        )
...
try:
    rows = cur.execute("""
    SELECT id,email,name,gender,show_me,city,lat,lon,email_verified,profile_photo_key
    FROM users
    WHERE email LIKE 'tester_female_%@celestya.test'
    ORDER BY email
    LIMIT 12
    """).fetchall()
