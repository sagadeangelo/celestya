...
print(f"Connecting to DB: {DB} (SEED MALE TESTERS)")
...
try:
    ...
    LIMIT 1
    """).fetchone()
    photo_key = row[0] if row and row[0] else "uploads/test_male_placeholder.jpg"
except Exception as e:
    print(f"Error checking photo key: {e}")
    photo_key = "uploads/test_male_placeholder.jpg"
...
print(f"Seeding {N} MALE users...")
for i in range(1, N + 1):
    email = f"tester_male_{i:02d}@celestya.test"
    name = f"Tester Hombre {i:02d}"
    ...
    if exists:
        ...
        insert_vals = [
            email, name, "male", "female", birth, age_bucket, 
...
# report
try:
    male_count = cur.execute("SELECT COUNT(*) FROM users WHERE gender='male'").fetchone()[0]
    print("MALE COUNT:", male_count)
except Exception:
    print("Could not count males")
