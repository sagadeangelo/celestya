...
class _VoiceIntroWidgetState extends State<VoiceIntroWidget> {
  ...
  void _startRecording() async {
    ...
    if (success) {
      ...
      setState(() => _error = 'Error al iniciar grabación. Verifica el permiso de micro.');
    }
  }
  ...
  void _togglePlayback() async {
    ...
    if (_isPlaying) {
      ...
      await _audioService.playAudio(_recordedPath!);
  ...
  void _saveRecording() {
    if (_recordedPath != null) {
      final updatedProfile = widget.profile.copyWith(voiceIntroPath: _recordedPath);
      widget.ref.read(profileProvider.notifier).updateProfile(updatedProfile);
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Icon(
                _isRecording ? Icons.mic_rounded : Icons.audiotrack_rounded, 
                color: _isRecording ? Colors.red : CelestyaColors.starDust
              ),
              const SizedBox(width: 8),
              Text(
                _recordedPath != null ? 'Tu presentación' : 'Preséntate con tu voz',
              ...
              child: GestureDetector(
                ...
                child: Column(
                  children: [
                    ...
                    const SizedBox(height: 12),
                    const Text('Toca para grabar', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w500)),
                  ],
                ),
              ),
            ),
            ...
            Center(
              child: Column(
                children: [
                  ...
                  const SizedBox(height: 12),
                  const Text('Toca para detener', style: TextStyle(color: Colors.redAccent)),
                ],
              ),
            ),

          if (_recordedPath != null && !_isRecording)
            // Review State: Play, Discard, Save
            Column(
              children: [
                Container(
                  padding: const EdgeInsets.all(16),
